...
public class TestCollectionSpeed {
    ...
    /*1. почитай про формат csv. это такие упрощенные таблицы, которые удобно парсить и их понимает эксель.
            2. getResults - тройной вложенный цикл. ну совсем не ок.
            3. проверка contains странно работает для LL и AL. непоследовательно
    4. формат времени, может быть, стоит использовать один(стандарт - мс)
    5. куча одинаковых методов. для исправления можно применить знания из тем "абстракция" или "дженерики".
    вспомнить слово generalization в контексте поведения(методов).
            6. if (j == 4) - ну magic number же
    7. блин, повыноси все нужные цифры в константы. плохо читается код.*/
}
